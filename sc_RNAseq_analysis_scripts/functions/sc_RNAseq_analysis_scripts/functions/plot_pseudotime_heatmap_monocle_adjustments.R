# Make improvements on the plot_pseudotime_heatmap() from the monocle package 
# in order to show pheudotime and state annotations
# set the state numbers and the number of gene clusters

plot_pseudotime_heatmap_2 <- function(cds_subset,
  ...
  ph_res <- pheatmap(heatmap_matrix[, ], #ph$tree_row$order
                     ...
                     annotation_colors = annotation_colors,    ## the annotation col changes are made here
                     drop_levels = FALSE,
                     treeheight_row = 20,
                     breaks = bks,
                     fontsize = 6,
